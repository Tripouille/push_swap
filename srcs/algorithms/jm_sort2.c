...
static bool	is_ready_to_finish(t_stacks *stacks)
{
	return (ilist_is_globally_sort2(&stacks->a, false)
			...
			&& (stacks->a.size == 0
			|| ilist_get_smallest(&stacks->a)->i > ilist_get_biggest(&stacks->b)->i));
}
			...
			t_stacks *stacks, t_slist *r_instructions)
{
	...
	//dprintf(2, " in put_on_b : a = "); ilist_show(&stacks->a, ' ');
	//dprintf(2, " in put_on_b : b = "); ilist_show(&stacks->b, ' ');
}
...
static t_slist	sort(t_instruction_infos const instructions[], t_stacks *stacks)
{
	...
	//dprintf(2, "before finish a = "); ilist_show(&stacks->a, ' ');
	//dprintf(2, "before finish b = "); ilist_show(&stacks->b, ' ');
	finish_sorting_ordered_stacks(instructions, &required_instructions,
		stacks);
	//dprintf(2, "after finish a = "); ilist_show(&stacks->a, ' ');
	//dprintf(2, "after finish b = "); ilist_show(&stacks->b, ' '); dprintf(2, "\n");
	return (required_instructions);
}

t_slist	jm_sort2(t_instruction_infos const instructions[], t_stacks *stacks)
{
	return (sort(instructions, stacks));
}
