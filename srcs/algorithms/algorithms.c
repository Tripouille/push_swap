...
void	stock_and_call(t_instruction_infos const instructions[],
	t_slist *required_instructions, char const *name, t_stacks *stacks)
{
...
t_ilist_element	*get_biggest_below(t_ilist *ilist, int value)
{
	...
	{
		if (element->i < value && (result == NULL || element->i > result->i))
			result = element;
							...
							t_slist *required_instructions, t_stacks *stacks)
{
	...
	{
		a_steps = get_r_lengths(&stacks->a, ilist_get_smallest(&stacks->a),
				stacks->a.head);
		b_steps = get_r_lengths(&stacks->b, ilist_get_smallest(&stacks->b),
				stacks->b.tail);
		best_rotates = get_best_rotates(&a_steps, &b_steps);
		execute_rotates(instructions, stacks, required_instructions,
			&best_rotates);
		while (!ilist_is_empty(&stacks->b))
			stock_and_call(instructions, required_instructions, "pa", stacks);
	}
